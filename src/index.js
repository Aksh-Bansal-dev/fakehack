...
const sparkline = grid.set(10, 10, 2, 2, contrib.sparkline, {
  label: "Network Bandwidth (bits/sec)",
  tags: true,
...
const map = grid.set(6, 0, 6, 6, contrib.map, {
...
//dummy data
const servers = ["US1", "US2", "EU1", "AS1", "AS2", "JP1"];
const commands = [
  "grep",
  "metasploit",
  "rm -rf",
...
setInterval(function () {
  ...
  else if (rnd == 2) log.log("Scanning all ports " + Math.random().toFixed(2));
  else if (rnd == 3) log.log("root access " + Math.random().toFixed(2));
  else if (rnd == 4) log.log("Terminating process " + Math.random().toFixed(2));
  screen.render();
}, 500);
...
function updateDonut() {
  ...
  donut.setData([
    {
      percent: parseFloat(((pct + 0.0) % 1).toString()).toFixed(2),
      label: "storage",
      color: color,
    },
  ]);
